...
<p class="muted">أفلام ومسلسلات وقنوات مباشرة، مع استيراد جماعي من رابط JSON/API أو بيانات JSON-LD العامة المصرح باستخدامها.</p>
...
<input id="url" placeholder="رابط التشغيل المصرح باستخدامه">
...
<h3>استيراد من رابط موقع / API</h3>

<p class="muted">
أدخل رابط JSON/API عام أو صفحة تحتوي JSON-LD منظمًا.
تعرض اللوحة النتائج أولًا ولا تحفظ شيئًا قبل اختيارك.
لا يتم تجاوز تسجيل الدخول أو الحماية.
</p>
...
placeholder='[{"title":"...","type":"movie","url":"https://..."}]'></textarea>
...
<h3>API</h3>
<p><code>GET /api/items?type=channel</code></p>
...
function typ(t){
 return t==='movie'?'فيلم':t==='series'?'مسلسل':'قناة';
}
...
async function load(){
 ...
 $('list').innerHTML=a.map(function(x){
  ...
  return '<div class="item">'+
   '<img class="poster" src="'+esc(x.poster||'')+'" onerror="this.style.visibility=\\'hidden\\'">'+
   '<div>'+
    '<b>'+esc(x.title)+'</b><br>'+
    '<span class="pill">'+typ(x.type)+'</span>'+
    category+
    '<span class="muted"> ID '+x.id+'</span>'+
   '</div>'+
   '<div class="actions">'+
...
async function stats(){
 ...
 ['movies','series','channels','sources'].forEach(function(k){
  $(k).textContent=s[k]||0;
 });
}
...
async function addItem(){
 ...
 let r=await adminFetch('/api/items',{
  method:'POST',
 ...
 $('msg').textContent=
  r&&r.ok?'تم الحفظ في D1':'تعذر الحفظ';
...
async function source(id){
 ...
 let url=prompt('رابط المصدر الإضافي');

 if(!url)return;

 let r=await adminFetch('/api/items/'+id+'/sources',{
  method:'POST',
  headers:{'content-type':'application/json'},
  body:JSON.stringify({
   url:url,
...
async function del(id){
 ...
 let r=await adminFetch('/api/items/'+id,{
  method:'DELETE'
 });
...
async function importJson(){
 ...
 try{
  ...
  let items=JSON.parse($('jsonbox').value);

  let r=await adminFetch('/api/import',{
   method:'POST',
   headers:{'content-type':'application/json'},
   body:JSON.stringify({items:items})
  });

  let j=r?await r.json():{};
...
async function previewRemote(){
 ...
 let r=await adminFetch('/api/import/preview',{
  method:'POST',
  headers:{'content-type':'application/json'},
  body:JSON.stringify({
   url:$('remoteUrl').value,
 ...
 if(!r||!r.ok){
  $('remoteMsg').textContent=j.error||'تعذر قراءة المصدر';
  return;
 }
 ...
 $('remoteMsg').textContent=
  'تم العثور على '+remoteItems.length+
  ' عنصر. راجع وحدد المطلوب.';

 $('preview').innerHTML=remoteItems.map(function(x,i){
...
function selectAll(v){
 document.querySelectorAll('.pick').forEach(function(x){
  x.checked=v;
 });
}
...
async function commitRemote(){
 ...
 let items=Array.from(
  ...
  return remoteItems[Number(x.dataset.i)];
 });
...
function normalize(raw,forced=''){
 ...
 return arr.map(x=>{
  ...
  let schema=clean(x['@type']).toLowerCase();
  let type=forced||clean(x.type).toLowerCase();
  ...
  let image=x.poster||x.image||x.logo||'';

  if(image&&typeof image==='object'){
   image=image.url||'';
  }

  let published=x.datePublished;
...
async function remotePreview(url,forced){
 ...
 try{
  ...
  throw Error('الرابط غير صالح');
 }

 if(!/^https?:$/.test(u.protocol)){
  throw Error('يسمح فقط بروابط HTTP/HTTPS');
 }

 let r=await fetch(u.toString(),{
  headers:{
   accept:'application/json,text/html;q=0.9',
   'user-agent':'MovieHub-Importer/1.0'
  },
  redirect:'follow'
 });

 if(!r.ok){
  throw Error('المصدر أعاد HTTP '+r.status);
 }
 ...
 if(text.length>3000000){
  throw Error('الاستجابة كبيرة جدًا');
 }
...
async function insert(env,x,skip){
 ...
 let r=await env.DB.prepare(
  'INSERT INTO items(title,type,poster,description,year,category,country) VALUES(?,?,?,?,?,?,?) RETURNING id'
 ).bind(
  clean(x.title),
  x.type,
  clean(x.poster),
 ...
 if(x.url){
  await env.DB.prepare(
   'INSERT INTO sources(item_id,label,url,priority) VALUES(?,?,?,1)'
  ).bind(
   r.id,
   'Primary',
   clean(x.url)
...
export default {
 async fetch(req,env){
  ...
  if(p==='/api/items'&&m==='GET'){
   ...
   let q=clean(u.searchParams.get('q'));
   let t=clean(u.searchParams.get('type'));

   let sql=
    'SELECT id,title,type,poster,description,'+
    'year,category,country,created_at '+
  ...
  let mm=p.match(/^\\/api\\/items\\/(\\d+)$/);

  if(mm&&m==='GET'){

   let x=await env.DB.prepare(
  ...
  mm=p.match(/^\\/api\\/items\\/(\\d+)\\/sources$/);
  ...
  if(mm&&m==='POST'){
   ...
   await env.DB.prepare(
    'INSERT INTO sources(item_id,label,url,priority) '+
    'VALUES(?,?,?,?)'
   ).bind(
    +mm[1],
